...
import mp4 from "./image2/WeChat_buzhou6.mp4";
//@ts-ignore
...
function Interaction6() {
  ...
  const { principal } = useSpring({
    from: { principal: 50000000 },

    to: { principal: startAnimation1 ? 50000000 : 49900000 },
    delay: 500,
    duration: 5000,
  });
  const { principal1 } = useSpring({
    from: { principal1: 0 },
    to: { principal1: startAnimation1 ? 0 : 10000 },
    delay: 500,
    duration: 5000,
  });
  ...
  const { payable } = useSpring({
    from: { payable: 0 },
    to: { payable: startAnimation1 ? 10000 : 0 },
    delay: 500,
    duration: 5000,
  });
  ...
  const next = () => {
    ...
    if (
      (text1 == "39e5b4830d4d9c14db7368a95b65d5463ea3d09520" &&
        qian1 == "10000") ||
      (text1 == "39e5b4830d4d9c14db7368a95b65d5463ea3d09520" &&
        qian1 == "10,000")
    ) {
      ...
      sessionStorage.setItem(
        ...
        JSON.stringify([
          ...arr,
          {
            ...
            payee: "刘琦",
            amount: "10,000",
            token: "39e5b4830d4d9c14db7368a95b65d5463ea3d09520",
          },
        ])
      );
    } else {
  ...
  const handleFormSubmit = (values: any) => {
    ...
    // handleModalClose();
    const isFormFilled =
      form.isFieldsTouched() && Object.keys(form.getFieldsValue()).length > 0;
    if (isFormFilled) {
      console.log(1); // 表单已填写
      generateHash("10000")
        .then((hash: any) => {
          setTokenList([{ key: "刘琦token", hash }]);
        }) // 输出：
        .catch((error) => console.error(error));
      message.success("获取hash成功，请到账户下查看！");
    } else {
      console.log(2); // 表单未填写
      message.warning("您没有填写任何东西！");
    }
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      >
        <Form id="modalForm" form={form} onFinish={handleFormSubmit}>
          <Form.Item
            name="username"
            label="账号"
            rules={[
          ...
          >
            <Input placeholder="请输入您交易资金的购买或者发放的实际用途" />
          </Form.Item>

          <Form.Item name="transactionAddress" label="地址">
              ...
              <div style={{ width: "90%", marginRight: "0.5vw" }}>
                ...
                <p style={{ fontSize: "1vw", lineHeight: "1.8vw" }}>
                  {isModalOpen ? (
                    <TypingCard
                      source={
                        "场景角色：本流程中学员将扮演“新优集团”企业财务的角色。<br/><br/>场景描述： ”新优集团“通过专项账户的数字人民币资金向员工支付工资。<br/><br/>实验操作步骤：学员需要分别在”新优集团“企业账户区域，输入对于个人发放的金额10,000元，以及每个金额相对应的token密钥。token的获取需要点击开始拨付按钮下方的“获取token按钮，完成资料的填写既可以获取token！”"
                      }
                    />
                  ) : (
                    ""
      ...
      >
        <Button
          onClick={() => {
            ...
            // next();
            startNext
              ? navigate("/admin/experimentalCenter/course/interaction7")
        ...
        >
          ...
          >
            ...
            >
              <img src={qiye1} style={{ width: "100%" }} alt="" />
              <p style={{ textAlign: "center", fontSize: "1vw" }}>新优集团</p>
            </div>
            <div
              style={{
                // height: "400px",
          ...
          >
            <div style={{ width: "10vw", height: "12vw" }}>
              ...
              <p style={{ textAlign: "center", fontSize: "1vw" }}>刘琦账户</p>
            </div>
